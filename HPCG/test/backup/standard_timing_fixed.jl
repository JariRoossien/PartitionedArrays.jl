...
delay = length(ARGS) >= 3 ? parse(Float64, ARGS[3]) : 0.001

println("Using dot product delay of $(delay*1000) ms")

# Define a delayed dot product function
function delayed_dot(a, b)
    result = dot(a, b)
    sleep(delay)  # Add delay to simulate network latency
    return result
end
...
# Define the PPCGIterable struct (copied from pipelined_cg.jl)
...
# Modified iterate function that uses delayed_dot instead of dot
function Base.iterate(it::PPCGIterable, iteration::Int = start(it))
    ...
    it.timing_data[1] += @elapsed begin
        if iteration == 0
            ...
            # g₀ = (w₀, u₀)
            dot_wu = delayed_dot(it.w, it.u)  # Use delayed_dot
            # d₀ = (m₀, w₀) 
            dot_mw = delayed_dot(it.m, it.w)  # Use delayed_dot
