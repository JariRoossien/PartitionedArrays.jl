...
# Include HPCG module for ref_cg!
include("src/HPCG.jl")
using .HPCG
...
"""
function hpcg_ref_cg!(x, A, b, nprocs; tolerance=1e-6, maxiter=1000)
...
"""
Standard CG implementation without MPI simulation (true reference)
"""
function standard_cg!(x, A, b, nprocs; tolerance=1e-6, maxiter=1000)
    n = length(x)
    r = b - A * x
    p = copy(r)
    ...
    while iter < maxiter && residual / residual0 > tolerance
        ...
        # Pure CG without any MPI simulation
        Ap = A * p
...
function perform_benchmarks(distribute_func, filename::String)
    ...
    # Benchmark both algorithms with clean slate - HPCG ref_cg vs pipelined MPI
    x0_ref = copy(x0)  # Fresh copy for reference algorithm
    ref_results = benchmark_mpi_cg_algorithm("HPCG Reference CG", hpcg_ref_cg!, A, b, x0_ref, total_procs;
                                            tolerance=tolerance, maxiter=maxiter, runs=runs)
   
    x0_pipe = copy(x0)  # Fresh copy for pipelined algorithm
    pipelined_results = benchmark_mpi_cg_algorithm("Pipelined CG (MPI)", pipelined_mpi_cg!, A, b, x0_pipe, total_procs;
                                                  tolerance=tolerance, maxiter=maxiter, runs=runs)
